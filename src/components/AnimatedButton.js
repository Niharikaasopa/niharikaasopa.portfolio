...
export default function AnimatedButton({ link, color }) {
  return (
    <button
      onClick={link}
      style={{ color: color }}
      className="relative inline-block px-8 py-6 group"
    >
      {/* Outer Circle */}
      <span
        className="absolute inset-0 w-12 h-12 rounded-full shadow-lg transition-transform duration-300 group-hover:scale-0"
